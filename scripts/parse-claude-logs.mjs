...
const MODEL_PRICING_USD_PER_1M = {
  ...
  "claude-opus-3-7": { input: 15, cachedInput: 1.5, output: 75 },
  "claude-sonnet-3-7": { input: 3, cachedInput: 0.3, output: 15 },
  "claude-haiku-3-5": { input: 0.8, cachedInput: 0.08, output: 4 },
  "claude-opus-3-5": { input: 15, cachedInput: 1.5, output: 75 },
  "claude-sonnet-3-5": { input: 3, cachedInput: 0.3, output: 15 },
  "claude-haiku-3": { input: 0.25, cachedInput: 0.03, output: 1.25 },
  "claude-opus-3": { input: 15, cachedInput: 1.5, output: 75 },
  "claude-sonnet-3": { input: 3, cachedInput: 0.3, output: 15 },
  default: { input: 3, cachedInput: 0.3, output: 15 },
};
...
function addMetrics(target, source) {
  ...
  target.effectiveInputTokens +=
    ...
    Math.max(0, (source.inputTokens || 0) - (source.cachedInputTokens || 0));
...
function finalizeMetricBucket(bucket) {
  ...
  bucket.cacheRatio =
    bucket.inputTokens > 0 ? bucket.cachedInputTokens / bucket.inputTokens : 0;
...
function handleTokenCount(event, session, day, date, detectedModel) {
  ...
  turn.effectiveInputTokens =
    (turn.effectiveInputTokens || 0) + (input - cached);
  ...
  const step = {
    ...
    totalTokens: total,
    cacheRatio: input > 0 ? cached / input : 0,
    tools: { ...toolAccum },
...
for (const file of files) {
  ...
  for (const line of lines) {
    ...
    if (isUserMessage(event)) {
      ...
      // Check for tool_result blocks (these are continuations, not new turns)
      const toolResults = extractToolResultBlocks(event);
      if (toolResults.length > 0 && !text) {
        // Pure tool result — accumulate file context but don't start new turn
...
const sessionList = Object.values(sessions)
  ...
  .map((session) => {
    ...
    for (const turn of session.turns || []) {
      ...
      turn.cacheRatio =
        turn.inputTokens > 0 ? turn.cachedInputTokens / turn.inputTokens : 0;
  ...
  return finalizeMetricBucket(
    createMetricBucket({
      ...
      effectiveInputTokens:
        turn.effectiveInputTokens ??
        Math.max(0, (turn.inputTokens || 0) - (turn.cachedInputTokens || 0)),
