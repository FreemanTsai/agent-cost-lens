...
const PUBLIC = 'public';
const MONITOR_REPARSE_INTERVAL_MS = parseInt(process.env.MONITOR_REPARSE_INTERVAL_MS, 10) || 10000;
...
function runDateOnlyReparse() {
  return new Promise((resolve, reject) => {
    execFile(process.execPath, [codexParserScript, '--date-only'], { cwd: rootDir }, (err, stdout, stderr) => {
      if (err) {
...
function refreshMonitorDataIfStale() {
  ...
  const now = Date.now();
  if (now - lastMonitorReparseAt < MONITOR_REPARSE_INTERVAL_MS) return Promise.resolve(null);
